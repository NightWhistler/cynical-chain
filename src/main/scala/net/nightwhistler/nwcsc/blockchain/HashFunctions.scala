...
object SimpleSHA256Hash extends HashFunction {
  def apply( block: Block ): BigInt = calculateHash(block.index, block.previousHash, block.timestamp, block.foundBy, block.messages, block.nonse)

  private def calculateHash(index: Long, previousHash: BigInt, timestamp: Long, foundBy: String, messages: Seq[BlockMessage], nonse: Long) =
